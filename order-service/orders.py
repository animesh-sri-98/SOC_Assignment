...
client = MongoClient("mongodb-container", 27017)
db = client["orders_db"]
...
def get_product_details_from_order(order_id):
    ...
    if order:
        product_id = order["product_id"]
        product_url = f'http://product-microservice:9996/products/{product_id}'
...
def get_user_details_from_order(order_id):
    ...
    if order:
        user_id = order["user_id"]
        user_url = f'http://user-microservice:9998/users/{user_id}'
